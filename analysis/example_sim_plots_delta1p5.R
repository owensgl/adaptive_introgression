...
library(grid)
library(gridExtra)
out3 <- read_tsv("Documents/adaptive_introgression/example_sim_jan29_delta1p5_out3.txt",
                 col_names = c("spacer","version","type", "gen","pop","mut_pos",
                               "mut_popID","mut_freq","mut_sel")) %>%
  filter(gen > 1)

out2 <- read_tsv("Documents/adaptive_introgression/example_sim_jan29_delta1p5_out2.txt",
...
plot_phenosum <- out3 %>%
  ...
  theme_few() + 
  geom_segment(aes(x=10000,xend=10100,y=0,yend=150),color=brewer.pal(3,"Set1")[1]) +
  facet_grid(pop~.) +
  xlab("Generation") +
  ylab("Phenotype")+
  theme(legend.position="bottom") +
...
pdf("Documents/adaptive_introgression/example_sim_delta1p5.plots.v2.pdf",width=15,height=8)
