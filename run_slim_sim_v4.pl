...
my %p;
$p{k} = 1000; #Population size
$p{m} = 0.01; #Mutation rate
$p{total_div} = 0.9; #Total amount of reproductive isolation amongst all divergently selected sites.
$p{div_n} = 100; #Number of divergently selected alleles
$p{div_s} = $p{total_div}/$p{div_n}; #Strength of selection at divergently selected alleles
...
$p{delta} = 0.1; #How fast the optimum shifts after the burn in
my $reps =10;  #Number of repetitions per set of parameters
$p{burn_in_gen} = 1000; #Number of generations of burn in before shift
...
foreach my $varying_parameter (sort keys %varying_p){
	foreach my $p_change (@p_changes){
		...
		$tmp_p{div_s} = $tmp_p{total_div}/$tmp_p{div_n};
