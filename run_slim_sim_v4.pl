...
foreach my $varying_parameter (sort keys %varying_p){
	foreach my $p_change (@p_changes){
		...
		foreach my $rep (1..$reps){
			...
			my $output_1 = `$slim -d d_pop_size=$tmp_p{k} -d d_m=$tmp_p{m} -d d_div_sel_n=$tmp_p{div_n} -d d_div_sel_s=$tmp_p{div_s} -d d_bdm_s=$tmp_p{bdm_s} -d d_bdm_n=$tmp_p{bdm_n} -d d_mutation_rate=$tmp_p{mutation_rate} -d d_delta=$tmp_p{delta} -d d_rep=$rep -d d_seed=$seed -d d_burn_in_gen=$tmp_p{burn_in_gen} -d d_shift_gen=$tmp_p{shift_gen} -d d_version=\\"burn\\" adaptive_introgression_WF_BDM_CMD.slim`;
			chomp($output_1);
                        my @lines = split(/\n/,$output_1);
			...
			my $output_2 = `$slim -d d_pop_size=$tmp_p{k} -d d_m=$tmp_p{m} -d d_div_sel_n=$tmp_p{div_n} -d d_div_sel_s=$tmp_p{div_s} -d d_bdm_s=$tmp_p{bdm_s} -d d_bdm_n=$tmp_p{bdm_n} -d d_mutation_rate=$tmp_p{mutation_rate} -d d_delta=$tmp_p{delta} -d d_rep=$rep -d d_seed=$seed -d d_burn_in_gen=$tmp_p{burn_in_gen} -d d_shift_gen=$tmp_p{shift_gen} -d d_version=\\"test\\" adaptive_introgression_WF_BDM_CMD.slim`;
			chomp($output_2);
			@lines = split(/\n/,$output_2);
			foreach my $line (@lines){
				if ($line =~ m/^OUT/){
			...
			my $output_3 = `$slim -d d_pop_size=$tmp_p{k} -d d_m=$tmp_p{m} -d d_div_sel_n=$tmp_p{div_n} -d d_div_sel_s=$tmp_p{div_s} -d d_bdm_s=$tmp_p{bdm_s} -d d_bdm_n=$tmp_p{bdm_n} -d d_mutation_rate=$tmp_p{mutation_rate} -d d_delta=$tmp_p{delta} -d d_rep=$rep -d d_seed=$seed -d d_burn_in_gen=$longer_gen -d d_shift_gen=0 -d d_version=\\"test\\" adaptive_introgression_WF_BDM_CMD.slim`;
			chomp($output_3);
			@lines = split(/\n/,$output_3);
			foreach my $line (@lines){
				if ($line =~ m/^OUT/){
